...
public class TokenObfuscatorTest {
	...
	public void doesNotLeakTheTokenItself() {
		...
		assertFalse("the obfuscated form must not be the token", token.equals(obfuscated));
		assertFalse("the algorithm prefix must not stay readable", obfuscated.startsWith("A:"));
		assertTrue("everything but the tail is masked", obfuscated.startsWith("****"));
	}
