...
public class AeadAuthTokenEncryptionAlgorithmTest {
	...
	public void testTokensAreShorterThanBlowfish() {
		...
		AesGcmAuthTokenEncryptionAlgorithm aes = new AesGcmAuthTokenEncryptionAlgorithm();
		aes.customize(PHRASE);
		assertTrue("aes-gcm token should be shorter than the blowfish one", aes.encryptAuthToken(token).length() < blowfishLength);
	}
	...
	private void testLinkSafety(AuthTokenEncryptionAlgorithm alg) {
		...
		for (int i = 0; i < 100; i++) {
			...
			assertTrue("token contains a character which can break a link: " + encrypted, TokenEncoding.isSafe(encrypted));
		}
	}
	...
	private void testRandomization(AuthTokenEncryptionAlgorithm alg) {
		...
		assertNotEquals("encrypting the same token twice must not produce the same string",
				alg.encryptAuthToken(token), alg.encryptAuthToken(token));
	}
