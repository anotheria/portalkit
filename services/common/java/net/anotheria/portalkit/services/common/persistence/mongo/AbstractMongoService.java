...
public abstract class AbstractMongoService {
	...
	protected synchronized void initialize() {
		...
		while (!mongoClientInitialized.get())
			try {
				...
				if (System.currentTimeMillis() > initStartTime + configuration.getInitMaxTime()) {
					...
					mongoClient.close();
					throw new RuntimeException("Can't initialize Mongo client.");
				}
		...
		if (database.isAuthenticate()) {
			...
			if (!authenticated)
				throw new RuntimeException("Can't authenticate for database[" + database.getName() + "] with username[" + database.getUsername() + "].");
		}
