...
public class OnlineStorageTest {
	...
	public void testLoginLogout() {
		...
		long lastLoginTime = getNanoTime();
		...
		try {
			storage.notifyLoggedIn(accountId, getNanoTime());
	...
	public void updateActivityTimeTest() {
		...
		try {
			storage.notifyActivity(accountId, getNanoTime());
		...
		final long lastLoginTime = getNanoTime();
		...
		final long lastActivityNanoTime = getNanoTime();
	...
	public void testReadLastActivityTimes() {
		...
		long activityUpdate = getNanoTime();
	...
	public void inactiveAccountsCleanUpTest() {
		...
		// ### Preparations Start
		final long expiredTimeStamp = getNanoTime() - (config.getMaxAccountInactivityInterval() * 1000000L) - 10000000L;
		...
		try {
			storage.notifyLoggedIn(aliveAcc, getNanoTime());
		} catch (AccountIsOnlineException e) {
			Assert.fail(e.getMessage());
		}
		// ### Preparations End!
	...
	private Map<AccountId, Long> buildLastLoginData(List<AccountId> accounts) {
		...
		for (AccountId account : accounts) {
			map.put(account, Math.abs(getNanoTime() + Math.abs(new Random(getNanoTime() + new Random(1000000L).nextLong()).nextLong()) + 2000000000));
			try {
				Thread.sleep(2L);
			} catch (InterruptedException e) {
				Assert.fail();
